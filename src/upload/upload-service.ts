...
@Injectable()
export class UploadService {//moze da se koristi kroz ceo Nestjs + deklarisemo 2 promenljive
  ...
  constructor(private readonly configService: ConfigService) {//configService koristimo da dohvatimo sve podatke iz .env
  ...
  async uploadPublicFile(file: Express.Multer.File): Promise<string> {
    ...
    await this.s3.send(
      new PutObjectCommand({ //saljemo PutObjectCommand sa sledecim info:
