...
export class AuctionQueryService {
  ...
  async getAuctions(auctionQueryDto: AuctionQueryDto) {
    ...
    if (auctionQueryDto.name) {
      //proverava da li korisnik unosi vrednost za nema(ime aukcije),ako korisnik nije uneo ime, blok se nece izvrsiti
      query.andWhere('auction.name LIKE :name', {
        name: `%${auctionQueryDto.name}%`,
      }); //auction.name-pozivamo polje name u tabeli auction
    ...
    if (auctionQueryDto.description) {
      query.andWhere('auction.description LIKE :description', {
        description: `%${auctionQueryDto.description}%`,
      });
    }

    if (auctionQueryDto.startingPrice) {
      query.andWhere('auction.startingPrice >= :startingPrice', {
        startingPrice: auctionQueryDto.startingPrice,
      });
    }

    if (auctionQueryDto.currentPrice) {
      query.andWhere('auction.currentPrice >= :currentPrice', {
        currentPrice: auctionQueryDto.currentPrice,
      });
    }

    if (auctionQueryDto.userId) {
      query.andWhere('auction.userId = :userId', {
        userId: auctionQueryDto.userId,
      });
    }

    if (auctionQueryDto.bidderId) {
      query.andWhere('auction.bidderId = :bidderId', {
        bidderId: auctionQueryDto.bidderId,
      });
    }

    if (auctionQueryDto.startDate) {
      query.andWhere('auction.startDate >= :startDate', {
        startDate: auctionQueryDto.startDate,
      });
    }

    if (auctionQueryDto.endDate) {
      query.andWhere('auction.endDate <= :endDate', {
        endDate: auctionQueryDto.endDate,
      });
    }
    ...
    const [auctions, totalAuctions] = await query
      ...
      .leftJoinAndSelect('auction.bids', 'bid', 'bid.amount > :amount', {
        amount: 0,
      })
      .getManyAndCount();
