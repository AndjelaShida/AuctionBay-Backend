  ...
  async autoBid(
    ...
    if (autoBidForThisUser) {
      throw new BadRequestException(
        'You already have automatic bid on this auction',
      );
    }
    ...
    const validBid = activeBid.filter(
      (autoBid) => autoBid.maxAmount > auction.currentPrice,
    );

    let lastCreatedBid: Bid | null = null;

    //iretiraj kroz sve validne autobudove
    for (const AutoBidEntity of validBid) {
      //Iteriramo kroz svaki pojedinačni automatski bid koji je prošao prethodnu filtraciju (validBid).
