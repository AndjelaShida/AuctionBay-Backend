...
async findImagesForAuction(@Param('auctionId') auctionId: number): Promise<Image[]> {
    ...
    if(!images || images.length === 0) {
        throw new NotFoundException( `No image found for auction with auction id ${auctionId}`)
    }
    return images ;
}
...
async findImagesForItem(@Param('itemId') itemId: number): Promise<Image[]> {
    ...
    if(!images || images.length === 0) {
        throw new NotFoundException( `No image found for item with auction id ${itemId}`)
    }
    return images ;
}
